...
export function createTools(onToolStart: (name: string) => void) {
  const run = async <T extends ExecutableTool["tool"]>(
    ...
    onToolStart(name);
    try {
      return await executeTool({ tool: name, ...args } as ExecutableTool);
    } catch (err) {
      // Return the error as a tool result instead of throwing — the model
      // can read it and decide whether to retry/recover. Throwing would
      // halt generation and surface a raw error to the user.
